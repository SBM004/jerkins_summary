    ...
    return (
        ...
        <nav className="text-sm   top-0 bg-white border-gray-200 flex flex-row items-center justify- w-full min-h-10   flex-nowrap ">
            ...
            <div className="leftcontainer pl-5 w-full flex flex-row items-center gap-3">
                ...
                <NavLink to="/dashboard" className={({ isActive }) => `flex flex-row items-center border-gray-200 pl-2 pr-2 py-1 hover:bg-blue-300 ${isActive && 'bg-blue-300'}`}>
                    <ChartIcon size={15} />
                    Dashboard
                </NavLink>

                <NavLink to="/" className={({ isActive }) => `flex flex-row items-center border-gray-200 pl-2 pr-2 py-1 hover:bg-blue-300 ${isActive && 'bg-blue-300'}`}>
                    <CubeIcon size={24} />
                    Package Status
                </NavLink>
                <NavLink to="/" className={`flex flex-row items-center border-gray-200 pl-2 pr-2 py-1 hover:bg-blue-300 ${() => isActive ? 'bg-blue-300' : 'bg-black'}`}>
                    <Activity size={24} />
                    Reports
                </NavLink>
                <NavLink to="/" className={`flex flex-row items-center border-gray-200 pl-2 pr-2 py-1 hover:bg-blue-300 ${() => isActive ? 'bg-blue-300' : 'bg-black'}`}>
